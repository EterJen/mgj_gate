...
Sidebar.prototype.init = function()
{
    ...
    var applyBtn = mxUtils.button(mxResources.get('apply'), function()
	{
		...
		var node = encoder.encode(sb.graph.getModel());
		mxUtils.popup(mxUtils.getXml(node), true);
	});
	applyBtn.className = 'geBtn gePrimaryBtn';
	outer.appendChild(applyBtn);
 ...
 */
Sidebar.prototype.thumbWidth = 60;
...
function DNode(orderNum,name,description,isAccept,isInitNode,isAbleToCancel,isAbleToReturn,candidateType,candidateExp,assigneeType,execType)
{
...
function DOperation(name,description,preCondition,postLogic)
{
	this.name = name;
	this.description = description;
...
var nodeCodec = new mxObjectCodec(new DNode());

nodeCodec.encode = function(enc, obj)
{
	var node = enc.document.createElement('DNode');
	mxUtils.setTextContent(node, JSON.stringify(obj));
	return node;
};

nodeCodec.decode = function(dec, node, into)
{
	var obj = JSON.parse(mxUtils.getTextContent(node));
	obj.constructor = DNode;
	return obj;
};

mxCodecRegistry.register(nodeCodec);

var operationCodec = new mxObjectCodec(new DOperation());

operationCodec.encode = function(enc, obj)
{
	var operation = enc.document.createElement('DOperation');
	mxUtils.setTextContent(operation, JSON.stringify(obj));
	return operation;
};

operationCodec.decode = function(dec, operation, into)
{
	var obj = JSON.parse(mxUtils.getTextContent(operation));
	obj.constructor = DOperation;
	return obj;
};

mxCodecRegistry.register(operationCodec);

Sidebar.prototype.addGeneralPalette = function(expand,elt)
{
	...
	var fns = [	 	
	 	this.addEntry(this.getTagsForStencil('general', 'user_task').join(' '), function()
		{
			var cell = new mxCell('节点名称', new mxGeometry(10, 10, 100, 100), 'text;html=1;strokeColor=#93C7FE;fillColor=#EAF4FE;align=center;verticalAlign=middle;whiteSpace=wrap;rounded=1');
			cell.vertex = true;
			cell.data = new DNode(0,'节点名称','默认描述',true,false,false,false,'All',null,'SinglePerson','Sequence');
			
			
			/*var cell1 = new mxCell('', new mxGeometry(0, 0, 20, 20), 'html=1;shape=mxgraph.bpmn.user_task;');
			cell1.vertex = true;
			cell1.geometry.relative = true;
			cell1.geometry.offset = new mxPoint(40, 32);
			cell.insert(cell1);

			var cell2 = new mxCell('操作1', new mxGeometry(0, 0, 22, 14), 'html=1;');
			cell2.vertex = true;
			cell2.geometry.relative = true;
			cell2.geometry.offset = new mxPoint(77,84);
			cell.insert(cell2);

			var cell3 = new mxCell('text', new mxGeometry(0, 0, 30, 20), 'text;html=1;strokeColor=none;fillColor=none;align=center;verticalAlign=middle;whiteSpace=wrap;rounded=0;');
			cell3.vertex = true;
			cell3.geometry.relative = true;
			cell3.geometry.offset = new mxPoint(33,6);
			cell.insert(cell3);*/
			
			return sb.createVertexTemplateFromCells([cell], cell.geometry.width, cell.geometry.height, 'User Task');
		}),
		this.createEdgeTemplateEntry('endArrow=classic;html=1;strokeColor=blue', 50, 50, '', '向前流转', null, lineTags + 'directional directed2'),
	 	this.createEdgeTemplateEntry('endArrow=classic;html=1;strokeColor=red', 50, 50, '', '退回', null, lineTags + 'directional directed'),
	    this.createEdgeTemplateEntry('endArrow=classic;html=1;strokeColor=gray', 50, 50, '', '回路', null, lineTags + 'directional directed1')
...
Sidebar.prototype.createItem = function(cells, title, showLabel, showTitle, width, height, allowCellsInserted)
{
	...
	//border.style.border='1px solid #94C8FF';
	if (mxClient.IS_IE6)
	{
...
Sidebar.prototype.createEdgeTemplate = function(style, width, height, value, title, showLabel, allowCellsInserted)
{
	...
	cell.data = new DOperation('操作','默认描述',true,null);
...
Sidebar.prototype.addPalette = function(id, title, expanded, onInit)
{
	...
	var elt = document.getElementById('sideBarFolder');
	
	var div = document.getElementById('sideBarThumbContainer'); 
	
	var outerDivForSideBar = document.getElementById('outerDivForSideBar');
...
/*	var outer = document.createElement('div');
    ...
    if (id != null)
    {
    	this.palettes[id] = [elt, outerDivForSideBar];
    }
