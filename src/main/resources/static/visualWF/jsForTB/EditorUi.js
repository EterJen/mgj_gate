...
EditorUi = function (editor, container, lightbox) {
    ...
    $.ajax({
        type: "POST",
        url: "open",
        data: "processVersionId=" + urlParams.processVersionId,
        success: function (resultInfo) {
            $(".flyover").hide();
            if (resultInfo != 'empty') {
                var doc = mxUtils.parseXml(resultInfo);
                var model = new mxGraphModel();
                var codec = new mxCodec(doc);
                codec.decode(doc.documentElement, model);
                var children = model.getChildren(model.getChildAt(model.getRoot(), 0));
                myEditorUI.editor.graph.setSelectionCells(myEditorUI.editor.graph.importCells(children));
            }
        },
    ...
    /*
         ...
         if(urlParams.mode=='create'){
           ...
           new mxXmlRequest(OPEN_URL,"processVersionId="+urlParams.processVersionId+"&mode="+urlParams.mode)
              .simulate(document);
      }*/
};
 ...
 */
EditorUi.prototype.formatWidth = 400;
 ...
 * screen widths <= 640px.
 */
EditorUi.prototype.hsplitPosition = (screen.width <= 640) ? 80 : 80;
...
EditorUi.prototype.init = function () {
    ...
    graph.getSelectionModel().addListener(mxEvent.CHANGE, mxUtils.bind(this, function () {
        function arrChange(a, b) {
            for (var i = 0; i < b.length; i++) {
                for (var j = 0; j < a.length; j++) {
                    if (a[j].id == b[i].id) {//如果是id相同的，那么a[ j ].id == b[ i ].id
                        a.splice(j, 1);
                        j = j - 1;
                    }
                }
        ...
        console.log('selection chagnede');
        var ngSelectedCellList = graph.getSelectionCells();
        var appElement = document.querySelector('[ng-controller=formCtrl]');
        var scope = angular.element(appElement).scope();

        if (ngSelectedCellList.length != 1) {
            scope.formPath = 'propertyForms/processInstanceProperty.html';
            scope.$apply();
            return;
        }
        var ngSelectedCell = ngSelectedCellList[0];

        console.log(ngSelectedCell);

        /*线条 初始化*/
        if ('DOperation' == ngSelectedCell.data.constructor.name) {

            /*意见必填默认值*/
            if (undefined == ngSelectedCell.data.optionValidate) {
                ngSelectedCell.data.optionValidate = false;
            }

            /*意见代签*/
            if (undefined == ngSelectedCell.data.optionAgent) {
                ngSelectedCell.data.optionAgent = false;
            }

            /*流转模式*/
            if (!ngSelectedCell.data.taskReturnMode) {
                ngSelectedCell.data.taskReturnMode = [];
            } else {
                var tempVar = [];
                if (ngSelectedCell.data.taskReturnMode.constructor == Array) {
                    ngSelectedCell.data.taskReturnMode.forEach(function (v) {
                        scope.taskReturnMode.forEach(function (v2) {
                            if (v.id == v2.id) {
                                tempVar.push(v2);
                            }
                        })
                    ...
                    /*兼容老的单选配置 xml中数据*/
                    scope.taskReturnMode.forEach(function (v2) {
                        if (ngSelectedCell.data.taskReturnMode == v2.id) {
                            tempVar.push(v2);
                        }
                    })
                }
                ngSelectedCell.data.taskReturnMode = tempVar;
            }

            /*参与者自动展开 可能多选 多种展开模式叠加*/
            if (!ngSelectedCell.data.candidatesExpandTypes) {
                ngSelectedCell.data.candidatesExpandTypes = [];
            } else {
                var chosedItems = [];
                var otherItems = [];
                ngSelectedCell.data.candidatesExpandTypes.forEach(function (v) {
                    var continueFlag = true;
                    scope.candidatesExpandTypes.forEach(function (v2) {
                        if (continueFlag && (v.id == v2.id)) {
                            chosedItems.push(v2);
                            continueFlag = false;
                        }
                    })
                ...
                arrChange(scope.candidatesExpandTypes, chosedItems);

                ngSelectedCell.data.candidatesExpandTypes = chosedItems;
                scope.candidatesExpandTypes = chosedItems.concat(scope.candidatesExpandTypes);
            }

            /*候选人排序 */
            if (!ngSelectedCell.data.candidatesOrder) {
                ngSelectedCell.data.candidatesOrder = [];
            } else {
                var chosedItems = [];
                var otherItems = [];
                ngSelectedCell.data.candidatesOrder.forEach(function (v) {
                    var continueFlag = true;
                    scope.candidatesOrder.forEach(function (v2) {
                        if (continueFlag && (v.id == v2.id)) {
                            chosedItems.push(v2);
                            continueFlag = false;
                        }
                ...
                arrChange(scope.candidatesOrder, chosedItems);

                ngSelectedCell.data.candidatesOrder = chosedItems;
                scope.candidatesOrder = chosedItems.concat(scope.candidatesOrder);
            }

            /*候选类型*/
            if (!ngSelectedCell.data.candiTypeOrder) {
                ngSelectedCell.data.candiTypeOrder = [];
            } else {
                var chosedItems = [];
                var otherItems = [];
                ngSelectedCell.data.candiTypeOrder.forEach(function (v) {
                    var continueFlag = true;
                    scope.candiTypeOrder.forEach(function (v2) {
                        if (continueFlag && (v.id == v2.id)) {
                            chosedItems.push(v2);
                            continueFlag = false;
                        }
                ...
                arrChange(scope.candiTypeOrder, chosedItems);

                ngSelectedCell.data.candiTypeOrder = chosedItems;
                scope.candiTypeOrder = chosedItems.concat(scope.candiTypeOrder);
            }


        }

        scope.ngSelectedCell = ngSelectedCell;

        if (scope.ngSelectedCell.data.selectedCandidateList == null)
            scope.ngSelectedCell.data.selectedCandidateList = [];
        if (scope.ngSelectedCell.vertex == '1') {
            scope.showNodeTab();
        } else if (scope.ngSelectedCell.edge == '1') {
            scope.formPath = 'propertyForms/edgePropertyForm.html';
        }
        scope.$apply();
...
EditorUi.prototype.refresh = function (sizeDidChange) {
    ...
    //this.sidebarContainer.style.background = '#E0EAFC';
    ...
    this.diagramContainer.style.background = '#E0EAFC';
...
EditorUi.prototype.createDivs = function () {
    ...
    this.formatContainer = document.getElementById('formatContainer');
    ...
    this.hsplit = document.getElementById('hsplit');
...
EditorUi.prototype.createUi = function () {
    ...
    this.format = document.getElementById('formatContainer');
        ...
        try {
            if (Editor.useLocalStorage) {
                ...
                if (xml.length < MAX_REQUEST_SIZE) {
                    /*$.ajax({
                                  type: "POST",
                                  url: "/save?processVersionId="+urlParams.processVersionId+"&mode="+urlParams.mode,
                                  data:'filename=' + encodeURIComponent(name) +'&xml=' + encodeURIComponent(xml),
                                  success: function(resultInfo) {
                                      //myEditorUI.openFile.setData(decodeURIComponent(encodeURIComponent(resultInfo)), 'dddd');
                                      alert(resultInfo);
                                  },
                                  error: function(XMLResponse) {
                                      console.log(JSON.stringify(XMLResponse));
                    ...
                    //new mxXmlRequest(SAVE_URL+"?processVersionId="+urlParams.processVersionId+"&mode="+urlParams.mode,'filename=' + encodeURIComponent(name) +'&xml=' + encodeURIComponent(xml)).simulate(document, '_blank');
                }
                else {
