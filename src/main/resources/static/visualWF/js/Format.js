...
Format.prototype.refresh = function()
{
	...
	else
	{
		...
		this.panels.push(new PropertyPanel(this, ui, propertiesPanel));
 ...
 * 添加属性表单
 */
PropertyPanel = function(format,editorUi,container){
	BaseFormatPanel.call(this, format, editorUi, container);
	this.init();
}

mxUtils.extend(PropertyPanel, BaseFormatPanel);
...
PropertyPanel.prototype.init = function()
{
	...
	var selectedCellList = graph.getSelectionCells();
	
	/*如果选择的cell的个数不是1个，则返回，无法进行属性的编辑*/
	if(selectedCellList.length!=1)
		return;
	
	var selectedCell = selectedCellList[0];
	
	var dataPropLabel = {};
	if(selectedCell.vertex=='1'){//如果是节点
		dataPropLabel = {
			//id是不可编辑的
			'orderNum':'节点序号',
			'name':'节点名称',
			...
			'isAccept':'是否需要接收',
			'isInitNode':'是否开始节点',
			'isAbleToCancel':'是否能撤销',
			'isAbleToReturn':'是否能退回',
			'candidateType':'参与者类型',
			'candidateExp':'参与者表达式',
			'assigneeType':'接收者类型',
			'execType':'执行类型'
		};
	}else if(selectedCell.edge=='1'){//如果是操作
		dataPropLabel = {
			//id是不可编辑的,
			//'name':'操作名称',
			'description':'操作描述',
			//targetNodeId是由可视化编辑器进行编辑的
			'preCondition':'前置条件',
			'postLogic':'后置逻辑',
			'preConditionFalseMessage':'迁移反馈消息'
		};
	}
	...
	var names = [];
	var texts = [];
	for (var prop in dataPropLabel){
		names.push(prop);
		var text = form.addText(dataPropLabel[prop],data[prop]);
		texts.push(text);
	}
	this.container.appendChild(form.table);
	...
	var applyBtn = mxUtils.button(mxResources.get('apply'), function()
	{
		...
		try
		{
			for (var i = 0; i < names.length; i++){
				data[names[i]] = texts[i].value;
				if(names[i]=='name'){
					selectedCell.value = data[names[i]];
				}
			}
			graph.refresh();
		}
