myApp.controller('leaveQueryCtrl', ['$sce','$rootScope', '$scope', 'ENV', '$state', 'SysUtils', 'maxHeigtTool', 'NodeTreeTool', function ($sce, $rootScope, $scope, ENV, $state, SysUtils, maxHeigtTool, NodeTreeTool) {
	...
	$scope.init = function () {
		SysUtils.requestByJsonSync('/restful/leave', {}, function (resultInfo) {
			$scope.leaveQuerySrc = $sce.trustAsResourceUrl(resultInfo.bean);
			$scope.$applyAsync();
		});
		$("#leaveQueryModal").modal('show');
	};
