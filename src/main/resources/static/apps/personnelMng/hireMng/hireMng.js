myApp.controller('hireMngCtrl', ['$rootScope', '$scope', 'ENV', '$state', 'SysUtils', 'maxHeigtTool', 'NodeTreeTool', function ($rootScope, $scope, ENV, $state, SysUtils, maxHeigtTool, NodeTreeTool) {

	console.log('借聘用管理');
	$rootScope.addBgground = true;
	$scope.queryBean = {paging: 'Yes', groupId: 'personnel', dbParams:{}};
	$scope.proInstList = [];
	...
	$scope.queryProInstList = function (modal) {
		...
		SysUtils.requestByJson("/rProcessInstance/queryPersonnelList", $scope.queryBean, function (resultInfo) {
			SysUtils.handleResult(resultInfo, {'state': $state}, function () {
	...
	$scope.openQueryModal = function (modal) {
		...
		$scope.queryBean.pageSize = $scope.paginationConf.itemsPerPage;
		$scope.queryBean.dbParams.temporaryHireType = $scope.temporaryHireTypes[0].key;
		$('#' + modal).modal('show');
	};
	...
	$scope.has = false;
	$scope.checkPersonnelAdminRole = function () {
		$scope.user = {};
		$scope.user.roleList = [];
		$scope.user.roleList.push({name: '临借聘报批管理员'});
