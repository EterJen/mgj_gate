myApp.controller('communicationDutyTableCtl', ['$rootScope', '$scope', 'ENV', '$state', 'SysUtils', '$timeout','$stateParams','dataFactory', function ($rootScope, $scope, ENV, $state, SysUtils, $timeout,$stateParams,dataFactory) {
    console.log("communicationDutyTableCtl controller==");
...
/*        if (type == 1) {
     ...
     * @param modal
     */
    $scope.saveUserInDuty = function (modal,type) {
    	
    	 $scope.fc.currentSelectDuty.initType=type;
    	 
    	 SysUtils.requestByJson("/dutytable/updateUsersTxbw",$scope.fc.currentSelectDuty, function (resultInfo) {
             SysUtils.handleResult(resultInfo, {'state': $state}, function () {
    ...
    $scope.queryUserBeanList = function (type) {

    	queryUserBean={department:type};
