myApp.controller('dutyStaffManagementCtl', ['$rootScope', '$scope', 'ENV', '$state', 'SysUtils', '$timeout','$stateParams','dataFactory', function ($rootScope, $scope, ENV, $state, SysUtils, $timeout,$stateParams,dataFactory) {
    console.log("dutyStaffManagementCtl controller==");
    ...
    $scope.newBean={};
    $scope.dataObject={
        departList:[{id:1,name:"技术保障处"},{id:2,name:"通信组"},{id:3,name:"办报组"}]
    };
    ...
    $scope.fc.getdepart = function (departId) {
        var departName="";
        $scope.dataObject.departList.forEach(function (value) {
            if (value.id == departId) {
    ...
    $scope.fc.addBean = function (modal) {
        ...
        $scope.newBean.department = $scope.queryBean.department;
    ...
    $scope.fc.deletBean = function (bean) {
        SysUtils.swalConfirm("提示",  "是否确定删除？", "info", function (isConfirm) {
            if (isConfirm) {
                SysUtils.requestByJson("/dutyuser/delete/"+bean.id,{}, function (resultInfo) {
                    SysUtils.handleResult(resultInfo, {'state': $state}, function () {
        ...
        if(!SysUtils.isEmpty($scope.dataObject.currentSelectBeanList)&&$scope.dataObject.currentSelectBeanList.length>0){
            SysUtils.swalConfirm("提示",  "是否确定批量删除？", "info", function (isConfirm) {
                if (isConfirm) {
                    ...
                    SysUtils.requestByJson("/dutyuser/batchDelete",param, function (resultInfo) {
                        SysUtils.handleResult(resultInfo, {'state': $state}, function () {
                            swal("提示",resultInfo.message,"info");
                            $scope.searchGoods();
                        })
