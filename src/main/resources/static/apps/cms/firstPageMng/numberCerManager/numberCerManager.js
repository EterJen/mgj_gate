myApp.controller('numberCerManagerCtl', ['$rootScope', '$scope', 'ENV', '$state', 'SysUtils', '$timeout', '$stateParams', '$filter', 'NodeTreeTool', 'dataFactory', '$interval', 'sessionStorageService', function ($rootScope, $scope, ENV, $state, SysUtils, $timeout, $stateParams, $filter, NodeTreeTool, dataFactory, $interval, sessionStorageService) {
    ...
    $scope.deletBean = function (bean) {
        SysUtils.swalConfirm('警告', '确认删除记录?', 'warning', function (isConfirm) {
            if (isConfirm) {
                SysUtils.requestByJson("/numberCertificateManager/delete/"+bean.id, {}, function (resultInfo) {
    ...
    $scope.addBeanCommit = function () {
        if (!SysUtils.notEmpty($scope.addOrUpdateBean.id)) {
            var url = '/numberCertificateManager/create';
        } else {
            var url = '/numberCertificateManager/update';
        }
    ...
    $scope.pageAuto = function () {
        ...
        var url = '/numberCertificateManager/list';
