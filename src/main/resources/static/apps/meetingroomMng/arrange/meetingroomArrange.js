myApp.controller('meetingroomArrangeCtrl', ['$scope', 'ENV', '$state', 'dataFactory', 'NodeTreeTool', 'SysUtils', function ($scope, ENV, $state, dataFactory, NodeTreeTool, SysUtils) {
	...
	$scope.calculatedHeight = function () {
		...
		$('#panel-body-table').css('height', $('.content').height() - $('#panel-heading').outerHeight(true) - $('#bmxx_tit').height() - $("#title").height());
		$('.table_shadow').css('max-height', $('.content').height() - $('#panel-heading').outerHeight(true) - $('#bmxx_tit').height() - $("#title").height());
	};
