myApp.controller('claimConfirmationCtl', ['$rootScope', '$scope', 'ENV', '$state', 'SysUtils', '$timeout','$stateParams', function ($rootScope, $scope, ENV, $state, SysUtils, $timeout,$stateParams) {
    console.log("claimConfirmationCtl controller=="+$stateParams.parameterName);
    ...
    $scope.fc.applicationList = function() {
        if($scope.dataObject.currentSelectBeanList!=null && $scope.dataObject.currentSelectBeanList.length>0){
            SysUtils.requestByJson("/goodsAG/batchUpdateStatus",$scope.dataObject.currentSelectBeanList, function (resultInfo) {
                SysUtils.handleResult(resultInfo, {'state': $state}, function () {
    ...
    $scope.pageAuto = function () {
        ...
        if (!SysUtils.notEmpty($rootScope.currentUser, ['username'])) {
            SysUtils.silenceWithAuthAjax("/coreUser/currentUser", {}, function (resultInfo) {
                ...
                $scope.queryBean.goodsApplication.applicationSectorId=$rootScope.currentUser.department.id;
                $scope.$applyAsync();
