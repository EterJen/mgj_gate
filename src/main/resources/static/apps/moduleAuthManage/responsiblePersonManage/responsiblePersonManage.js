...
myApp.controller('responsiblePersonManageCtrl', ['$scope', 'ENV', '$state', 'SysUtils', 'dataFactory', 'NodeTreeTool', function ($scope, ENV, $state, SysUtils, dataFactory, NodeTreeTool) {
...
// }
  ...
  $scope.delPost = function () {
    if (!$scope.selectedPost.id) {
      swal("请选择岗位", "请先删除该岗位下用户", "info");
    } else {
      if ($scope.postUsers.length > 0) {
        swal("该岗位下存在用户", "请先删除该岗位下用户", "info");
        return;
      }
  ...
  $scope.queryDeptsTree = function () {
    accUrl = ENV.localapi + "/coreDepartment/responsiblePersonManage/deptMixPostTree";
    dataFactory.getlist(accUrl, 'POST', {'Content-type': 'application/json'}, {"initType": "containResponsiblePerson"}).then(
            function (resultInfo) {
              SysUtils.handleResult(resultInfo, {'state': $state}, function () {
