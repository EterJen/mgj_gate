...
public class TestAttendanceD {
	...
	ResultInfo<AttendanceD> resultInfo = null;
	AttendanceD initBean = null;
	AttendanceD createdBean = null;
	AttendanceD updatedBean = null;
	BigDecimal createdBeanId = null;
	List<AttendanceD> beanList = null;
	...
	public void testCrud() throws Exception{
		//创建权限:初始化
		resultInfo = testUtils.request("/attendanceD/init?initType=create", JsonUtils.emptyJson,AttendanceD.class, unittestTokenStr, mockMvc);
		initBean = resultInfo.getBean();
		
		//设置随机值，新建一条记录
		testUtils.setInitValues(initBean,new String[]{"id"});
		resultInfo = testUtils.request("/attendanceD/create", JsonUtils.writeAsFlatJson(initBean),AttendanceD.class, unittestTokenStr, mockMvc);
		createdBeanId = resultInfo.getBeanId();
		initBean.setId(createdBeanId);
		
		//读取数据库
		resultInfo = testUtils.request("/attendanceD/read/"+createdBeanId, JsonUtils.emptyJson, AttendanceD.class, unittestTokenStr, mockMvc);
		...
		resultInfo = testUtils.request("/attendanceD/update", JsonUtils.writeAsFlatJson(createdBean), AttendanceD.class, unittestTokenStr, mockMvc);
		...
		//获取queryBean
		resultInfo = testUtils.request("/attendanceD/init?initType=query", JsonUtils.emptyJson,AttendanceD.class, unittestTokenStr, mockMvc);
		AttendanceD queryBean = resultInfo.getBean();
		queryBean.setId(updatedBean.getId());
		resultInfo = testUtils.request("/attendanceD/list", JsonUtils.writeAsFlatJson(queryBean), AttendanceD.class, unittestTokenStr, mockMvc);
		...
		//删除
		resultInfo = testUtils.request("/attendanceD/delete/"+createdBeanId, JsonUtils.emptyJson, AttendanceD.class, unittestTokenStr, mockMvc);
