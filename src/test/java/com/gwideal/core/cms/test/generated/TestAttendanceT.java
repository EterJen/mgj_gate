...
public class TestAttendanceT {
	...
	ResultInfo<AttendanceT> resultInfo = null;
	AttendanceT initBean = null;
	AttendanceT createdBean = null;
	AttendanceT updatedBean = null;
	BigDecimal createdBeanId = null;
	List<AttendanceT> beanList = null;
	...
	public void testCrud() throws Exception{
		//创建权限:初始化
		resultInfo = testUtils.request("/attendanceT/init?initType=create", JsonUtils.emptyJson,AttendanceT.class, unittestTokenStr, mockMvc);
		initBean = resultInfo.getBean();
		
		//设置随机值，新建一条记录
		testUtils.setInitValues(initBean,new String[]{"id"});
		resultInfo = testUtils.request("/attendanceT/create", JsonUtils.writeAsFlatJson(initBean),AttendanceT.class, unittestTokenStr, mockMvc);
		createdBeanId = resultInfo.getBeanId();
		initBean.setId(createdBeanId);
		
		//读取数据库
		resultInfo = testUtils.request("/attendanceT/read/"+createdBeanId, JsonUtils.emptyJson, AttendanceT.class, unittestTokenStr, mockMvc);
		...
		resultInfo = testUtils.request("/attendanceT/update", JsonUtils.writeAsFlatJson(createdBean), AttendanceT.class, unittestTokenStr, mockMvc);
		...
		//获取queryBean
		resultInfo = testUtils.request("/attendanceT/init?initType=query", JsonUtils.emptyJson,AttendanceT.class, unittestTokenStr, mockMvc);
		AttendanceT queryBean = resultInfo.getBean();
		queryBean.setId(updatedBean.getId());
		resultInfo = testUtils.request("/attendanceT/list", JsonUtils.writeAsFlatJson(queryBean), AttendanceT.class, unittestTokenStr, mockMvc);
		...
		//删除
		resultInfo = testUtils.request("/attendanceT/delete/"+createdBeanId, JsonUtils.emptyJson, AttendanceT.class, unittestTokenStr, mockMvc);
